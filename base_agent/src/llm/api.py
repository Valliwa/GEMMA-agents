...
class ModelProvider:
    ...
    @classmethod
    def _get_gemma_client(cls):
        """Get Gemma client."""
        return DummyClient(api_key="gemma")
    ...
    def get_provider(cls, provider: Provider):
        ...
        if provider not in cls._providers:
            match provider:
                ...
                case Provider.GEMMA:  # ADD THIS CASE
                     client = cls._get_gemma_client()
                     cls._providers[provider] = GemmaProvider(client)
                case _:
